...
//
//  Created by Kamil Bloch on 14/11/2019.
...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        scoreLabel.position = CGPoint(x: frame.midX/4, y: frame.midY/4)
    ...
    func checkTouches(_ touches: Set<UITouch>) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            guard node.name == "rocket" else { continue }
        ...
        for (index, firework) in fireworks.enumerated().reversed() {
            if firework.position.y > 900 {
                ...
                if numberOfLaunches > 10 {
                    ...
                    scoreLabel.position = CGPoint(x: frame.midX/2 + 200, y: frame.midY/2)
                    scoreLabel.color = .cyan
